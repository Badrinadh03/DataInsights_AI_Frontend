  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        // parse safely — backend may emit NaN (invalid JSON) for missing values
        const text = await res.text();
        let body: PreviewResponse = {};
        try {
          body = JSON.parse(text.replace(/:\s*NaN\b/g, ": null"));
        } catch {
          // non-JSON response; leave body empty so error path triggers
        }
